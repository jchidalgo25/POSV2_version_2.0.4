...
namespace POS.Control.Main.MainTouch
{
    public partial class frmMainTouchClte : Telerik.WinControls.UI.RadForm
    {
        ...
        public void ActualizarDatosCliente(ClienteEmpleado cliente, bool mostrarMensajeNoApp = true)
        {
            ...
            // ── Ajustar fuente según largo del nombre ──
            int largo = lblClienteVal.Text.Length;
            if (largo <= 25)
                lblClienteVal.Font = new Font("Segoe UI", 13, FontStyle.Regular);
            else if (largo <= 35)
                lblClienteVal.Font = new Font("Segoe UI", 11, FontStyle.Regular);
            else if (largo <= 45)
                lblClienteVal.Font = new Font("Segoe UI", 9, FontStyle.Regular);
            else
                lblClienteVal.Font = new Font("Segoe UI", 8, FontStyle.Regular);
            //lblDireccionVal.Text = cliente.DireccionCliente;
            //lblTelefonoVal.Text = cliente.TelefonoCliente;
            lblClubDelportalVal.Font = lblClienteVal.Font;
            lblBnfIvaVal.Font = lblClienteVal.Font;
        ...
        private void ConstruirLayoutPDF()
        {
            ...
            _webBanner.Navigate($"http://192.168.127.36:3030/banner.html?est={est}");
            // ─────────────────────────────────────────────────────────
        }
            ...
            int xLabel, int xValor, int y)
        {
            ...
            lblValor.AutoEllipsis = false;
            lblValor.ForeColor = Color.Black;
            lblValor.MaximumSize = new Size(420, 24);
        ...
        private void ConstruirPanelTotales(int ancho, int yInicio, int alto)
        {
            ...
            int anchoIzq = (int)(ancho * 0.58);   // ~58 %  → subtotal / descuento / IVA
            int anchoDer = ancho - anchoIzq;       // ~42 %  → TOTAL prominente

            // ══ PANEL IZQUIERDO ══════════════════════════════════════════════════════
            var pnlLeft = new Panel();
            pnlLeft.Location = new Point(0, 0);
            pnlLeft.Size = new Size(anchoIzq, alto);
            pnlLeft.BackColor = Color.White;
            pnlTotales.Controls.Add(pnlLeft);

            // DESPUÉS (reemplazar con esto):
            int xTit = 16;
            int xVal = anchoIzq - 150;
            // distribuir las 3 filas en el alto disponible con padding uniforme
            int margenV = (int)(alto * 0.10);          // ~10% arriba y abajo
            int espacioEntreFilas = (alto - margenV * 2) / 3;
            int y = margenV;

            CrearFilaTotalesEn(pnlLeft, "Subtotal", ref lblValorTit, ref lblValorVal, xTit, xVal, y, Color.Black); y += espacioEntreFilas;
            CrearFilaTotalesEn(pnlLeft, "Descuento", ref lblDescuentoTit, ref lblDescuentoVal, xTit, xVal, y, Color.FromArgb(56, 142, 60)); y += espacioEntreFilas;
            CrearFilaTotalesEn(pnlLeft, "IVA 15%:", ref lblIvaTit, ref lblIvaVal, xTit, xVal, y, Color.Black);

            // Mensaje de agradecimiento (parte inferior izquierda)
            //var lblGracias = new Label();
            //lblGracias.Text = "¡Gracias por su compra en Delportal!";
            //lblGracias.Font = new Font("Segoe UI", 9, FontStyle.Italic);
            //lblGracias.ForeColor = Color.FromArgb(100, 100, 100);
            //lblGracias.Location = new Point(xTit, alto - 26);
            //lblGracias.Size = new Size(anchoIzq - xTit - 8, 22);
            //lblGracias.TextAlign = ContentAlignment.MiddleLeft;
            //pnlLeft.Controls.Add(lblGracias);

            // ══ PANEL DERECHO (TOTAL prominente) ════════════════════════════════════
            var pnlRight = new Panel();
            pnlRight.Location = new Point(anchoIzq, 0);
            pnlRight.Size = new Size(anchoDer, alto);
            pnlRight.BackColor = Color.FromArgb(0, 102, 51);   // verde oscuro corporativo
            pnlTotales.Controls.Add(pnlRight);

            // Etiqueta "TOTAL A PAGAR"
            lblTotalTit = new Label();
            lblTotalTit.Text = "TOTAL A PAGAR";
            lblTotalTit.Font = new Font("Segoe UI", 11, FontStyle.Bold);
            lblTotalTit.ForeColor = Color.FromArgb(200, 230, 200);   // blanco verdoso
            lblTotalTit.TextAlign = ContentAlignment.MiddleCenter;
            lblTotalTit.Location = new Point(0, 8);
            lblTotalTit.Size = new Size(anchoDer, 26);
            pnlRight.Controls.Add(lblTotalTit);

            // Valor del total — protagonista visual
            lblTotalVal = new Label();
            lblTotalVal.Text = "$0.00";
            lblTotalVal.Font = new Font("Segoe UI", 28, FontStyle.Bold);
            lblTotalVal.ForeColor = Color.White;
            lblTotalVal.TextAlign = ContentAlignment.MiddleCenter;
            lblTotalVal.Location = new Point(0, 36);
            lblTotalVal.Size = new Size(anchoDer, alto - 70);
            pnlRight.Controls.Add(lblTotalVal);

            // Subtítulo "IVA Incluido" pequeño debajo del valor
            var lblIvaIncluido = new Label();
            lblIvaIncluido.Text = "IVA Incluido";
            lblIvaIncluido.Font = new Font("Segoe UI", 8, FontStyle.Regular);
            lblIvaIncluido.ForeColor = Color.FromArgb(180, 220, 180);
            lblIvaIncluido.TextAlign = ContentAlignment.MiddleCenter;
            lblIvaIncluido.Location = new Point(0, alto - 20);
            lblIvaIncluido.Size = new Size(anchoDer, 18);
            pnlRight.Controls.Add(lblIvaIncluido);
        }

        private void CrearFilaTotalesEn(Panel parent, string etiqueta,ref Label lblEtiq, ref Label lblVal,int xTit, int xVal, int y, Color colorValor)
        {
            lblEtiq = new Label();
            lblEtiq.Text = etiqueta;
            lblEtiq.Font = new Font("Segoe UI", 13, FontStyle.Bold);
            lblEtiq.ForeColor = Color.Black;
            lblEtiq.Location = new Point(xTit, y);
            lblEtiq.Size = new Size(180, 30);
            lblEtiq.TextAlign = ContentAlignment.MiddleLeft;
            parent.Controls.Add(lblEtiq);

            lblVal = new Label();
            lblVal.Text = "$0.00";
            lblVal.Font = new Font("Segoe UI", 13, FontStyle.Bold);
            lblVal.ForeColor = colorValor;
            lblVal.Location = new Point(xVal, y);
            lblVal.Size = new Size(140, 30);
        ...
        private void InicializarWebViewPublicidad()
        {
            try
            {
                ...
                webViewAds.Navigate($"http://192.168.127.36:3030/carrusel.html?est={est}");
            }
            catch (Exception ex)
            {
                Control.Common.Logger.LogMessage(
            ...
            this.InvokeIfRequired(() =>
            {
                try
                {
                    ...
                    if (webViewAds != null)
                    {
                        ...
                        string est = Control.Common.GlobalParameters.Establecimiento ?? "";
                        string url = $"http://192.168.127.36:3030/carrusel.html?est={est}&t={DateTime.Now.Ticks}";
        ...
        private void InicializarTimerCarrusel()
        {
            ...
            _timerRecargarCarrusel.Tick += async (s, e) =>
            {
                try
                {
                    string url = "http://192.168.127.36:3030/api/data";
                    ...
                    using (var client = new System.Net.WebClient())
                    {
                        ...
                        url = $"http://192.168.127.36:3030/api/data?_t={DateTime.Now.Ticks}";
                            ...
                            if (!string.IsNullOrEmpty(_ultimoUpdatedAt) && nuevoTs != _ultimoUpdatedAt)
                            {
                                ...
                                this.InvokeIfRequired(() =>
                                {
                                    if (webViewAds != null)
                                    {
                                        ...
                                        string nuevaUrl = $"http://192.168.127.36:3030/carrusel.html?est={est}&t={DateTime.Now.Ticks}";
        ...
        private void InicializarTimerBanner()
        {
            ...
            _timerRecargarBanner.Tick += async (s, e) =>
            {
                try
                {
                    string url = "http://192.168.127.36:3030/api/banner/status";
                                ...
                                this.InvokeIfRequired(() =>
                                {
                                    if (_webBanner != null && !_webBanner.IsDisposed)
                                    {
                                        string est = Control.Common.GlobalParameters
                                            .Establecimiento ?? "";
                                        string nuevaUrl = $"http://192.168.127.36:3030/banner.html" +
